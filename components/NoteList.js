...
export default function NoteList({ categoryId }) {
  ...
  return (
    <Masonry
      items={notes}
      config={{ columns: [1, 2, 4], gap: [16, 16, 16], media: [768, 1280, 1920] }}
...
const Note = ({ note }) => {
  ...
  const [limit] = useState(() => limits[Math.floor(Math.random() * limits.length)]);
  ...
  const noteCategories = categories.filter((category) => note.categories?.includes(category.id));
